...
- **Running**: set a cadence (−/+, tap along with your steps, or presets from 150 to 180) and the current scene gets a kick on every step, hats in between and a pulsing bass. The music plays in half-time over a fixed four-chord loop, each chord held about 20 seconds. Changing cadence only changes the tempo. A looping silent media element keeps iOS playing with the screen locked.
