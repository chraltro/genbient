...
- **Listen · Run · Sleep** at the top are the three ways to use it. Under the scene name, each shows only its own few controls:
  ...
  - *Run*: cadence with − and +, Tap to set it from your steps, the section, interval round and run clock, a Skip button (skip the warm-up, or on to the next push or easy stretch), a Music row (Song chords, Fresh bass), intervals and run length. *More* opens the full running settings, including warm-up length.
...
- **Fresh bass**: a Groove bass pattern at the drum tempo. Eight 16-step grooves from sparse off-beats to rolling 16ths, using root, octave, fifth, seventh and third. It adds ghost notes, a run into every chord change and a fill every fourth bar, on a bright saw with a snappy filter. In a running song it is held through breakdowns and gets busier as the song lifts.
